# Hi, I'm Juan Esteban Calderon

I'm a software developer from Colombia with a professional background in the financial sector, particularly in risk management and investment-related systems.

I enjoy building backend applications, automation tools, and data-driven solutions that solve real business problems. Most of my current work focuses on Java, Python, SQL, and Linux-based environments.

I'm especially interested in:

- Backend engineering
- Financial and quantitative systems
- AI / Retrieval-Augmented Generation (RAG)
- DevOps and infrastructure
- Distributed systems and microservices
- Data engineering and analytics

Currently exploring cloud infrastructure, scalable system design, and self-hosted technologies.

---

## Technologies & Tools

<div align="center">

<img width="12%" src="https://www.vectorlogo.zone/logos/python/python-ar21.svg">
<img width="12%" src="https://www.vectorlogo.zone/logos/java/java-ar21.svg">
<img width="12%" src="https://www.vectorlogo.zone/logos/mysql/mysql-ar21.svg">
<img width="12%" src="https://www.vectorlogo.zone/logos/docker/docker-ar21.svg">
<img width="12%" src="https://www.vectorlogo.zone/logos/amazon_aws/amazon_aws-ar21.svg">
<img width="12%" src="https://www.vectorlogo.zone/logos/apache/apache-official.svg">

</div>

---

## Current Focus

- Building backend and microservice-based applications
- AI-powered document retrieval systems
- Financial data processing and analytics
- Linux server environments and automation
- DevOps practices and infrastructure tooling

---

## GitHub Stats
[![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=juancalderonde)](https://github.com/anuraghazra/github-readme-stats)
